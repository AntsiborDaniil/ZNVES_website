...
const CheckoutForm = ({ onOrderSubmit, showRightColumn = true, className = "" })=>{
    ...
    const handleSubmitOrder = async ()=>{
        ...
        try {
            ...
            // Получаем warehouse_item id для каждого товара
            const positions = await Promise.all(items.map(async (item)=>{
                ...
                if (!fullProduct?.slug) {
                    // Если нет slug, используем productId как UUID
                    return {
                ...
                try {
                    // Загружаем детали товара для получения warehouse_item id
                    // Для этого нужно загрузить полную информацию о товаре через API
                    const productResponse = await fetch(`http://62.84.115.11:8000/api/catalog/${fullProduct.slug}/`);
                    if (productResponse.ok) {
                        const productData = await productResponse.json();
                        if (productData.warehouse_items) {
                            // Ищем warehouse_item по color и size
                            // color и size могут быть в формате slug
                    ...
                    console.warn(`Failed to get warehouse_item for product ${fullProduct.slug}:`, error);
                }
                // Fallback: используем slug как основу для UUID
                return {
...
//# sourceMappingURL=src_6d5783f4._.js.map